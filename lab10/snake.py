...
def load_config(filename='lab10/database2.ini', section='postgresql'):
    parser = ConfigParser()
...
cur.execute("create table if not exists table1(user_name varchar(20), user_score varchar(20), user_level varchar(20));")
m=input("Enter an username: ")
score = 0; level = 1; 
cur.execute("SELECT * FROM table1 where user_name = %s;", (m,))
res = cur.fetchone()
insert_check = True
if res:
    print("current level of user:", res[2])
    insert_check = False
...
def game_over():
    ...
    if insert_check:
        cur.execute("insert into table1 (user_name, user_score, user_level) values(%s, %s, %s);",(m, score, level))
        conn.commit()
    cur.close()
    conn.close()
